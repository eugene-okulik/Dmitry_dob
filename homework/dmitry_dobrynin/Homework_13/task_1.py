...
for data_line in read_files():
    ...
    if "на неделю позже" in data_line.lower():
        result = date_data_line + datetime.timedelta(days=7)
        print(f'Задание было «распечатать эту дату, но на неделю позже», результат: {result}')
        ...
        days_ago = (time_now - date_data_line).days
        print(f'Задание было «распечатать сколько дней назад была эта дата», результат: {days_ago}')
    else:
